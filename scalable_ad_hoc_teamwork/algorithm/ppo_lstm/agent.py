...
from scalable_ad_hoc_teamwork.algorithm.ppo_lstm.networks.network_interaction_monolith \
    import InteractionPPONetwork as InteractionPPONetworkMonolith
from scalable_ad_hoc_teamwork.algorithm.ppo_lstm.networks.network_interaction_split_full_state \
    import InteractionPPONetwork as InteractionPPONetworkSplitFullState
from scalable_ad_hoc_teamwork.algorithm.ppo_lstm.networks.network_interaction_split_no_lstm \
    import InteractionPPONetwork as InteractionPPONetworkSplitNoLSTM
...
class Architecture(Enum):
    MONOLITH = "Monolith"
    SEPARATED = "Separated"
    SPLIT_FULL_STATE = "SplitFullState"
    SPLIT_NO_LSTM = "SplitNoLSTM"
...
class Agent:
    ...
    def __init__(self, obs_dim_state, obs_dim_agent, n_actions, memory_size, num_epochs, learning_rate,
                 num_batches, gamma, state_segmentation_func, preprocessor, interaction_architecture, entropy_coeff,
                 vf_clip_param, clip_param, kl_coeff, vf_loss_coeff, device):
        self.task_models = {}
    ...
    def select_action(self, state, **kwargs):
        with torch.no_grad():
            ...
            tensor_list = [torch.from_numpy(arr) for arr in task_weights + interaction_weights]
            tensor_probs = torch.cat(tensor_list, dim=0)
            tensor_list = [torch.from_numpy(arr) for arr in task_credit_weights + interaction_credit_weights]
            tensor_credit_weights = torch.cat(tensor_list, dim=0)
            categorical = self.compute_distribution(tensor_probs, tensor_credit_weights)
    ...
    def get_interaction_weights(self, state):
        ...
        for other_id, task in self.other_agent_tasks.items():
            ...
            credit_values = np.full_like(logits, values.item())
            credit_weights = credit_values / np.sum(credit_values)
    ...
    def add_interaction(self, interaction):
        if self.architecture == Architecture.MONOLITH:
            ...
            new_model = InteractionPPONetwork(self.obs_dim_state, self.obs_dim_agent * 2,
                                              self.obs_dim_agent + self.n_actions, self.n_actions,
                                              self.learning_rate, self.device)
        elif self.architecture == Architecture.SPLIT_FULL_STATE:
            new_model = InteractionPPONetworkSplitFullState(self.obs_dim_state, self.obs_dim_agent * 2,
                                                            self.obs_dim_agent + self.n_actions, self.n_actions,
                                                            self.learning_rate, self.device)
        elif self.architecture == Architecture.SPLIT_NO_LSTM:
            new_model = InteractionPPONetworkSplitNoLSTM(self.obs_dim_state, self.obs_dim_agent * 2,
                                                         self.obs_dim_agent + self.n_actions, self.n_actions,
                                                         self.learning_rate, self.device)
        else:
    ...
    def learn_step(self, **kwargs):
        if self.memory.full():
            if self.mode is self.LEARN_TASK:
                return self.learn_step_task()
            elif self.mode is self.LEARN_INTERACTION:
                return self.learn_step_interaction()
            else:
                return {}
        else:
            return {}
    ...
    def learn_step_task(self):
        ...
        return self.core_learn_step(agents, agent_configs, ppo_surrogate_loss, active_model)
    ...
    def learn_step_interaction(self):
        ...
        last_old_hidden_state = active_model._old_hidden_state
        agents = {f"player_0": active_model}

        agent_1_config = {"task_segmentation_func": seg_func, "agent_id": self.agent_id,
                          'other_agent_id': list(self.other_agent_tasks.keys())[0], "gamma": self.gamma,
                          "vf_clip_param": self.vf_clip_param, "clip_param": self.clip_param,
                          "kl_coeff": self.kl_coeff, "vf_loss_coeff": self.vf_loss_coeff,
                          "entropy_coeff": self.entropy_coeff}
        agent_configs = {"player_0": agent_1_config}
        postprocess(agents, self.memory, agent_configs)

        stats = self.core_learn_step(agents, agent_configs, ppo_surrogate_loss, active_model)
        active_model._hidden_state = old_hidden_state
        active_model._old_hidden_state = last_old_hidden_state
        return stats
    ...
    def core_learn_step(self, agents, agent_configs, ppo_surrogate_loss, active_model):
        ...
        self.memory.reset()
        return {"mean_entropy": np.mean(mean_entropies)}
