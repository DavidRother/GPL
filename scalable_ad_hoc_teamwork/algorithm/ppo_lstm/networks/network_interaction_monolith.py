...
class InteractionPPONetwork(nn.Module):
    ...
    def __init__(self, in_dim_state, in_dim_agents, in_dim_other_agent, out_dim, lr=0.0001, device="cpu"):
        ...
        self._hidden_type_layers = nn.LSTM(in_dim_other_agent + in_dim_state + in_dim_agents, self.hidden_size)
    ...
    def forward(self, obs_state, obs_agent, obs_other_agent, hidden_state=None):
        if obs_other_agent.dim() == 3:
            ...
            obs_agent = obs_agent.unsqueeze(0)
            concat_input = torch.cat((obs_state, obs_agent, obs_other_agent), dim=2)
        else:
            concat_input = torch.cat((obs_state, obs_agent, obs_other_agent), dim=1)
        if hidden_state is not None:
            self._hidden_state = hidden_state
            agent_type_output, self._hidden_state = self._hidden_type_layers(concat_input, self._hidden_state)
            if agent_type_output.dim() == 3:
                agent_type_output = agent_type_output.squeeze(0)
        else:
            agent_type_output, self._hidden_state = self._hidden_type_layers(concat_input, self._hidden_state)
        self._output = self._hidden_composite_layer(agent_type_output)
