...
class InteractionPPONetwork(nn.Module):
    ...
    def __init__(self, in_dim_state, in_dim_agents, in_dim_other_agent, out_dim, lr=0.0001, device="cpu"):
        ...
        self._hidden_type_layers = nn.LSTM(in_dim_other_agent, self.hidden_size)
        self._hidden_composite_layer = nn.Sequential(
            nn.Linear(256 * 2 + self.hidden_size, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 256),
    ...
    def forward(self, obs_state, obs_agent, obs_other_agent, hidden_state=None):
        ...
        if hidden_state is not None:
            self._old_hidden_state = hidden_state
            agent_type_output, self._hidden_state = self._hidden_type_layers(obs_other_agent, self._old_hidden_state)
            if agent_type_output.dim() == 3:
                agent_type_output = agent_type_output.squeeze(0)
            self._agent_type_output = agent_type_output
        else:
            self._old_hidden_state = self._hidden_state
            self._agent_type_output, self._hidden_state = self._hidden_type_layers(obs_other_agent,
                                                                                   self._old_hidden_state)
    ...
    def select_action(self, obs_state, obs_agent, obs_other_agent):
        ...
        return action, logits_numpy, self._old_hidden_state
