...
GT_OTHER_STATE = "gt_other_state"
GT_OTHER_STATE_NEXT = "gt_other_state_next"
...
class MultiAgentBuffer:
    ...
    def commit(self, obs, actions, next_obs, rewards, done, truncation, info, logits, hidden_state,
               last_other_agents_action, next_last_other_agents_action, step):
        for player in obs:
            self.buffer[player].commit(obs[player], actions[player], next_obs[player], rewards[player], done[player],
                                       truncation[player], info[player], logits[player], hidden_state[player],
...
class Buffer:
    ...
    def commit(self, obs, actions, next_obs, rewards, done, truncation, info, logits, hidden_state,
        ...
        self.data_struct[TERMINATIONS].append(done)
        self.data_struct[TRUNCATIONS].append(truncation)
    ...
    def buffer_episodes(self):
        buf_list = []
        done_indices = [0] + [i + 1 for i, x in enumerate(zip(self.data_struct[TERMINATIONS],
                                                              self.data_struct[TRUNCATIONS])) if x[0] or x[1]]
