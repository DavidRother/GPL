...
from scalable_ad_hoc_teamwork.algorithm.ppo_tom_architecture.learning.buffer import *
from scalable_ad_hoc_teamwork.algorithm.ppo_tom_architecture.util import *
...
def postprocess(agents, ma_buffer, agent_configs):
    ...
    for player in agents:
        ...
        ma_buffer.buffer[player] = reconstruct_buffer(ma_buffer, new_buf_list, player)

    return ma_buffer


def augment_state(ma_buffer, agent_configs, player):
    other_agent_id = agent_configs[player]["other_agent_id"]
    state_segmentation_func = agent_configs[player]['task_segmentation_func']
    action_prediction_model = agent_configs[player]['action_prediction_model']
    ...
    hidden_state_list = [h for h in ma_buffer.buffer[player][HIDDEN_STATES]]
    ...
    state_agents_next = []
    state_other_next = []
    gt_state_other_next_list = []
    next_action_list = [a for a in ma_buffer.buffer[player][LAST_OTHER_AGENTS_ACTION_NEXT]]

    for s, a, h, n_s, n_a in zip(obs_list, action_list, hidden_state_list,
                                 ma_buffer.buffer[player][NEXT_OBS].detach().cpu().numpy(), next_action_list):
        composite = state_segmentation_func(s.reshape(1, -1), other_agent_id, a)
        next_composite = state_segmentation_func(n_s.reshape(1, -1), other_agent_id, n_a)
        with torch.no_grad():
            action_prediction, _ = action_prediction_model(composite[0].squeeze(), composite[2].squeeze(), h)
            gt_state_other = composite[2].squeeze()
            other_agent_state = composite[2].squeeze().copy()
            other_agent_state[-action_prediction.shape[1]:] = action_prediction.detach().cpu().numpy()
            next_action_prediction, _ = action_prediction_model(next_composite[0].squeeze(),
                                                                next_composite[2].squeeze())
            gt_state_other_next = next_composite[2].squeeze()
            next_other_agent_state = next_composite[2].squeeze().copy()
            next_other_agent_state[-next_action_prediction.shape[1]:] = next_action_prediction.detach().cpu().numpy()
        world_state.append(composite[0].squeeze())
        state_agents.append(composite[1].squeeze())
        state_other.append(other_agent_state)
        gt_state_other_list.append(gt_state_other)

        world_state_next.append(next_composite[0].squeeze())
        state_agents_next.append(next_composite[1].squeeze())
        state_other_next.append(next_other_agent_state)
        gt_state_other_next_list.append(gt_state_other_next)
    ...
    ma_buffer.buffer[player][GT_OTHER_STATE] = gt_state_other_list
    ...
    ma_buffer.buffer[player][GT_OTHER_STATE_NEXT] = gt_state_other_next_list


def reconstruct_buffer(ma_buffer, new_buffer_list, player):
    new_buffer = Buffer(ma_buffer.buffer[player].size)
    for buf in new_buffer_list:
