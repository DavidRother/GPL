...
class InteractionPPONetwork(nn.Module):
    ...
    def __init__(self, in_dim_state, in_dim_agents, in_dim_other_agent, out_dim, lr=0.0001, device="cpu"):
        ...
        self._hidden_layers = nn.Sequential(
            nn.LSTM(in_dim_state + in_dim_agents + in_dim_other_agent, 512),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 256),
            nn.ReLU(),
    ...
    def forward(self, obs, hidden_state=None):
        self._output, self._hidden_state = self._hidden_layers(obs, hidden_state)
        if hidden_state is not None:
            self._hidden_state = hidden_state
            agent_type_output, self._hidden_state = self._hidden_type_layers(obs_other_agent, self._hidden_state)
            if agent_type_output.dim() == 3:
                agent_type_output = agent_type_output.squeeze(0)
            self._agent_type_output = agent_type_output
        else:
            self._agent_type_output, self._hidden_state = self._hidden_type_layers(obs_other_agent, self._hidden_state)
        self._output = self._hidden_composite_layer(torch.cat((self._state_output, self._agent_output,
                                                               self._agent_type_output), dim=1))
    ...
    def init_hidden(self):
        ...
        num_directions = 1
        batch_size = 1
        hidden_size = 128  # this matches the number of hidden units in the LSTM

        # Initialize hidden state and cell state to zeros
        h0 = torch.zeros(num_layers * num_directions, hidden_size, device=self.device)
        c0 = torch.zeros(num_layers * num_directions, hidden_size, device=self.device)
