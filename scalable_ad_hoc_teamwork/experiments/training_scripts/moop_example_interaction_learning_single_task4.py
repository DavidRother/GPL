...
folder_path = "evalpy"
experiment_identifier = "lbf_test4"
...
with moop_agent.eval_mode():
    eval_stats = evaluate(eval_env, [moop_agent, h1_agent], 100)
...
with evalpy.start_run(experiment_name):
    pbar = tqdm.tqdm(train(env, [moop_agent, h1_agent], num_episodes))
    ...
    for stats in pbar:
        ...
        #     break
    with moop_agent.eval_mode():
        eval_stats = evaluate(eval_env, [moop_agent, no_op_agent], 100)
