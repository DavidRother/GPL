...
experiment_name = f"ppo_experiment_{experiment_identifier}_{seed}"
...
eval_env = env_creator.get_env_instance(testing_identifier)
...
if checkpoint:
    ...
    ppo_agent = agent_creator.get_agent_instance("ppo", obs_dim=obs_dim, n_actions=n_actions, n_agents=n_agents,
                                                 device=device)
    ppo_agent.add_task("collect_apples")
ppo_agent.switch_active_tasks(["collect_apples"])
...
eval_stats = evaluate(eval_env, [ppo_agent], 100)
...
with evalpy.start_run(experiment_name):
    pbar = tqdm.tqdm(train(env, [ppo_agent], num_episodes))
    old_episode_number = 0
    for stats in pbar:
        ppo_agent.learn_step()
        ...
        if stats["Episode Number"] > old_episode_number:
            ...
            if old_episode_number % 100 == 0:
                file_path = f"{project_path}models/ppo/temp/ppo_{experiment_identifier}_ep{old_episode_number}_seed{seed}.agent"
                ppo_agent.save_agent(file_path)
            if old_episode_number % 100 == 0:
                eval_stats = evaluate(eval_env, [ppo_agent], 100)
                last_eval_mean = np.mean(eval_stats["Last episode reward player_0"]).item()
                if last_eval_mean > best_full_eval_mean:
                    ppo_agent.save_agent(
                        f"{project_path}models/ppo/final_versions/ppo_best_save_{experiment_identifier}.agent")
...
ppo_agent.save_agent(f"{project_path}models/ppo/final_versions/ppo_{experiment_identifier}.agent")
