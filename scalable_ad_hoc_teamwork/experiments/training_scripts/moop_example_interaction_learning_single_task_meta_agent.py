...
folder_path = "evalpy"
experiment_identifier = "lbf_meta_test"
...
env = env_creator.get_env_instance(identifier, players=n_agents, tasks=tasks, obs_spaces=obs_spaces,
                                   force_coop=force_coop, normalize_reward=normalize_rewards)
...
with moop_agent.eval_mode():
    eval_stats = evaluate(eval_env, [moop_agent, super_agent], 100)
...
with evalpy.start_run(experiment_name):
    pbar = tqdm.tqdm(train(env, [moop_agent, super_agent], num_episodes))
